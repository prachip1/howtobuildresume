 ...
 */
export async function generateAcademicQuestions(context, existingResumeData = null) {
...
Your task: Generate ${existingResumeData ? '5-9' : '12-15'} smart, conversational questions that collect ${existingResumeData ? 'the academic information missing from the user\'s existing job resume' : 'all the information needed to build a strong resume for this specific application'}. The questions must reflect country-specific conventions and degree-specific priorities.
...
${existingResumeData ? `
EXISTING RESUME DATA (extracted from the user's uploaded job resume):
${JSON.stringify({
  personalInfo: existingResumeData.personalInfo,
  summary: existingResumeData.summary,
  workExperience: existingResumeData.workExperience,
  education: existingResumeData.education,
  skills: existingResumeData.skills,
  projects: existingResumeData.projects,
  certifications: existingResumeData.certifications,
}, null, 2)}

CRITICAL INSTRUCTIONS FOR EXISTING RESUME MODE:
- The user already has a job resume. Your job is to adapt it into an academic resume, NOT rebuild it from scratch.
- DO NOT ask for information that is already well-filled: skip questions about name, email, phone, location, work experience, and education if those sections already have data.
- DO ask about information that is MISSING or needs to be REFRAMED for academic applications:
  1. Academic summary — their professional summary must be rewritten as an academic bio/objective for ${degreeLabel} in ${countryLabel}. Always ask for this.
  2. Research experience — a job resume won't have this. Always ask.
  3. Education grade in the correct format for ${countryLabel} — the parsed GPA may be wrong format or missing.
  4. Thesis / capstone project title if relevant to ${degreeLabel}.
  5. Publications, conference papers, or posters (not in a job resume).
  6. Honors, scholarships, academic awards (not in a job resume).
  7. Language skills with proficiency levels in the format required by ${countryLabel}.
  8. Any missing academic-specific context for ${degreeLabel} in ${countryLabel}.
- Generate 5-9 focused questions (fewer than scratch because much is already filled).
- Be encouraging — mention that their existing background will be reused.
` : ''}
IMPORTANT RULES:
